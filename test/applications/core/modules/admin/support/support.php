...
class _support extends \IPS\Dispatcher\Controller
{
	...
	public function _selfService( $data )
	{
		...
		{			
			...
			return new \IPS\Helpers\MultipleRedirect(
				...
				{
					/* MR data returns NULL on the first run */
					$thingToTry = (int) $thingToTry;
	...
	public function _upgradeCheck( $id )
	{
		try
		{
			...
			if ( $response['longversion'] > \IPS\Application::load('core')->long_version and $response['version'] != \IPS\Application::load('core')->version )
			{
