...
class _register extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$steps['basic_info'] = function ( $data ) use ( $self )
		{
			...
			if ( $values = $form->values() )
			{
				...
				if ( \IPS\Settings::i()->security_questions_enabled and \IPS\Settings::i()->security_questions_prompt === 'register' and ( !\IPS\Settings::i()->security_questions_can_opt_out or !$values['security_questions_optout_title'] ) )
				{
					$answers = array();
	...
	public static function buildRegistrationForm()
	{
		...
		/* Security Questions */
		if ( \IPS\Settings::i()->security_questions_enabled and \IPS\Settings::i()->security_questions_prompt === 'register' )
		{
			...
			foreach ( range( 1, min( $numberOfQuestions, count( $securityQuestions ) ) ) as $i )
			{
				$securityValidation = function( $val ) {
					if ( !$val and ( !\IPS\Settings::i()->security_questions_can_opt_out or !isset( \IPS\Request::i()->security_questions_optout_title_checkbox ) ) )
					{
