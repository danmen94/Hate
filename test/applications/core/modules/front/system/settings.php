...
class _settings extends \IPS\Dispatcher\Controller
{
	...
	protected function _email()
	{
		...
		if ( $output = \IPS\MFA\MFAHandler::accessToArea( 'core', 'EmailChange', \IPS\Http\Url::internal( 'app=core&module=system&controller=settings&area=password', 'front', 'settings_password' ) ) )
		{
			return $output;
		}
	...
	protected function _password()
	{
		...
		if ( $output = \IPS\MFA\MFAHandler::accessToArea( 'core', 'PasswordChange', \IPS\Http\Url::internal( 'app=core&module=system&controller=settings&area=password', 'front', 'settings_password' ) ) )
		{
			return $output;
		}
	...
	/**
	 * Security Questions Forgot
	 *
	 * @return	string
	 */
	protected function securityquestionsforgot()
	{
		/* If we have an existing validation record, we can just reuse it */
		$sendEmail = TRUE;
		try
		{
			$existing = \IPS\Db::i()->select( array( 'vid', 'email_sent' ), 'core_validating', array( 'member_id=? AND forgot_security=1', \IPS\Member::loggedIn()->member_id ) )->first();
			$vid = $existing['vid'];
		...
		{
			$vid = md5( \IPS\Member::loggedIn()->members_pass_hash . \IPS\Login::generateRandomString() );
			...
			\IPS\Db::i()->insert( 'core_validating', array(
				...
				'entry_date'  		=> time(),
				'forgot_security'   => 1,
				'ip_address'  		=> \IPS\Member::loggedIn()->ip_address,
				'email_sent'  		=> time(),
			) );
		}
		...
		if ( $sendEmail )
		{
			\IPS\Email::buildFromTemplate( 'core', 'forgotSecurityAnswers', array( \IPS\Member::loggedIn(), $vid ), \IPS\Email::TYPE_TRANSACTIONAL )->send( \IPS\Member::loggedIn() );
			$message = "forgot_security_confirm";
		}
		else
		{
			$message = "forgot_security_too_soon";
		}
		
		/* Show confirmation page with further instructions */
		\IPS\Output::i()->sidebar['enabled'] = FALSE;
		\IPS\Output::i()->bodyClasses[] = 'ipsLayout_minimal';
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('security_questions');
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'system' )->securityAnswersResetConfirm( $message );
	}
	
	/**
	 * Validate Reset Security Answers
	 *
	 * @return	void
	 */
	protected function securityquestionsvalidate()
	{
		/* Validate */
		try
		{
		...
		{
			\IPS\Output::i()->error( 'security_no_validation_key', '2C122/K', 410, '' );
		}
