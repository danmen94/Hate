...
class _Purchase extends \IPS\Node\Model
{
	...
	{				
		...
		{
			$description[] = (string) $this->renewals;
		}
	...
	{
		if ( $this->billing_agreement and $this->billing_agreement->status() != \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED )
		{
			return FALSE;
		}
	...
	public function canRenewUntil( \IPS\Member $member = NULL, $inCycles = FALSE, $admin = FALSE )
	{
		...
		if ( $this->cancelled or !$this->renewals or ( $this->billing_agreement and $this->billing_agreement->status() != \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) )
		{
			return FALSE;
		}
	...
	public function canCancel( \IPS\Member $member = NULL )
	{
		...
		return !$this->grouped_renewals and ( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and $this->renewals and $this->active;
	}
	...
	public function buttons( $ref='v' )
	{
		...
		if ( !$this->grouped_renewals and ( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and $this->renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'payments', 'invoices_add' ) )
		{
			$return['renew'] = array(
				'icon'	=> 'refresh',
		...
		if ( ( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and $this->parent() and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_edit' ) )
		{
			if ( !$this->grouped_renewals )
			{
				$return['group'] = array(
		...
		if( ( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_transfer' ) )
		{
		...
		if ( ( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_cancel' ) )
		{
			if ( $this->cancelled )
			{
		...
		if ( !( !$this->billing_agreement or $this->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_delete' ) )
		{
			$return['delete'] = array(
				'icon'	=> 'times-circle',
				'title'	=> 'delete',
