...
class _Package extends \IPS\Node\Model
{
	...
	public function clientAreaPage( \IPS\nexus\Purchase $purchase )
	{
		...
		{
			/* Reactivate */
			if ( !$purchase->renewals and count( json_decode( $this->renew_options, TRUE ) ) and $purchase->can_reactivate and ( !$purchase->billing_agreement or $purchase->billing_agreement->status() == \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) )
			{
	...
	{
		if ( $this->custom or !$purchase->active or ( !$this->allow_upgrading and !$this->allow_upgrading and !$overrideRestrictions ) or $purchase->grouped_renewals or ( $purchase->billing_agreement and $purchase->billing_agreement->status() != \IPS\nexus\Customer\BillingAgreement::STATUS_CANCELED ) )
		{
			return array();
		}
		else
