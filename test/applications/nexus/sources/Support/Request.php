...
class _Request extends \IPS\Content\Item implements \IPS\Content\ReadMarkers
{
	...
	public function nextPrevious( $type=0, $excludeAssignedToOther=FALSE )
	{
		if ( !array_key_exists( intval( $type ), $this->nextPrevious ) )
		{
			...
			if ( isset( \IPS\Request::i()->cookie['support_dpt_group'] ) and \IPS\Request::i()->cookie['support_dpt_group'] )
			{
				foreach ( \IPS\Db::i()->select( 'dpt_id', 'nexus_support_departments', array( ( $type == 1 ? 'dpt_position<?' : 'dpt_position>?' ), $this->department->position ), ( $type == 1 ? 'dpt_position DESC' : 'dpt_position ASC' ) ) as $departmentId )
				{
	 ...
	 * @param	\IPS\Member|NULL	The member, or FALSE if there is no member
	 * @return	void
	 */
	public function log( $action, $old, $new, \IPS\Member $member = NULL )
	{
		$log = new \IPS\nexus\Support\Log;
		$log->request = $this;
		if ( $member === FALSE )
