...
class _info extends \IPS\Dispatcher\Controller
{
	 ...
	 * @return	array
	 */
	protected function _buildInfoForm()
	{
		$formElements = array();
		$formElements['cm_first_name']	= new \IPS\Helpers\Form\Text( 'cm_first_name', \IPS\nexus\Customer::loggedIn()->cm_first_name, TRUE );
		$formElements['cm_last_name']	= new \IPS\Helpers\Form\Text( 'cm_last_name', \IPS\nexus\Customer::loggedIn()->cm_last_name, TRUE );
		foreach ( \IPS\nexus\Customer\CustomField::roots() as $field )
		{
			$column = $field->column;
			$formElements[ $column ] = $field->buildHelper( \IPS\nexus\Customer::loggedIn()->$column );
		}
