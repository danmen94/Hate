...
class _view extends \IPS\Dispatcher\Controller
{
	...
	protected function standing()
	{
		...
		$comparisons = \IPS\Db::i()->select( 'AVG(xval) AS avg, MAX(xval) AS max, MIN(xval) AS min', \IPS\Db::i()->select( "{$sum} as xval", 'nexus_support_requests', 'r_member<>0', NULL, NULL, 'joined', 'xval>0' ) )->join( 'core_members', 'member_id=r_member' )->first();
