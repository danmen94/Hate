...
abstract class _Session
{
	...
	public function setMember( $member )
	{
		...
		/* When we are setting a new member, regenerate session ID */
		$this->regenerateId();
	}

	/**
	 * @brief	Set a flag when we regenerate the ID to prevent an infinite loop
	 */
	protected $regenerated	= FALSE;

	/**
	 * When a user's access level is elevated, we regenerate a session ID to prevent session fixation attacks
	 * 
	 * @return	void
	 */
	public function regenerateId()
	{
		/* If we've already regenerated, stop now */
		if( $this->regenerated === TRUE )
		{
			return;
		}

		/* Set our flag that we are regenerating */
		$this->regenerated	= TRUE;

		/* Back up the current session data */
		$currentSessionData	= $_SESSION;
		$member				= $this->member;

		/* Mark the current session as destroyed. If someone tries to access the old session again we will treat them as a guest. */
		$_SESSION['sessionDestroyed']	= time();

		/* Generate a new session ID */
		session_regenerate_id();

		/* Obtain the new session ID */
		$sessionId	= session_id();

		/* Start the session back up with the new ID */
	...
	public function init()
	{
		...
		/* Security check to prevent session fixation attack ... */
		if( isset( $_SESSION['sessionDestroyed'] ) AND $_SESSION['sessionDestroyed'] < time() - 300 AND $this->regenerated === FALSE )
		{
			/* This shouldn't happen - a session that we have destroyed when we regenerated an ID should not be re-requested. This code
				would be hit if we were being attacked with a session fixation attack, or if the network connection is unstable */
