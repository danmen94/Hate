...
class _Front extends \IPS\Session
{
	...
	public function read( $sessionId )
	{
		...
		if ( !$this->member->member_id and isset( \IPS\Request::i()->cookie['member_id'] ) and isset( \IPS\Request::i()->cookie['pass_hash'] ) )
		{
			try
			{
				...
				{
					/* Regenerate a session ID to prevent session fixation attacks. We have to set a flag and then regenerate the ID
						after $this->data is set, otherwise saving won't work properly. */
		...
		/* Regenerate our session ID if necessary, but not if we have already done so */
		if( $regenerateSessionId AND $this->regenerated === FALSE )
		{
			$this->regenerateId();
		}
	...
	public function regenerateId()
	{
		...
		/* Let the parent method do its thing */
		parent::regenerateId();
