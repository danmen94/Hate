...
class _Output
{
	...
	public function sendOutput( $output='', $httpStatusCode=200, $contentType='text/html', $httpHeaders=array(), $cacheThisPage=TRUE, $pageIsCached=FALSE, $parseFileObjects=TRUE )
	{
		...
		if ( ( \IPS\QUERY_LOG or \IPS\CACHING_LOG ) and ( $contentTypeTypes[0] == 'text' OR ( $contentTypeTypes[0] == 'application' AND $contentTypeTypes[1] == 'json' ) ) )
		{
	...
	public function showBanned()
	{
		...
		$this->sendOutput( \IPS\Theme::i()->getTemplate( 'global', 'core' )->globalTemplate( $this->title, $this->output, array( 'app' => \IPS\Dispatcher::i()->application->directory, 'module' => \IPS\Dispatcher::i()->module->key, 'controller' => \IPS\Dispatcher::i()->controller ) ), 403 );
	}
