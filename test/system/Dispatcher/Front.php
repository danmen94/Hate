...
class _Front extends \IPS\Dispatcher\Standard
{
	...
	public function init()
	{
		...
		{
			/* Need their name, email, or answers to security questions... */
			...
			if ( ( \IPS\Settings::i()->security_questions_enabled and !\IPS\Member::loggedIn()->members_bitoptions['has_security_answers'] and \IPS\Settings::i()->security_questions_prompt === 'register' and ( !\IPS\Settings::i()->security_questions_can_opt_out or !\IPS\Member::loggedIn()->members_bitoptions['security_questions_opt_out'] ) ) and $this->controller !== 'settings' and $this->controller !== 'contact' and $this->controller !== 'privacy' )
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=system&controller=settings&area=securityquestions&initial=1', 'front', 'settings_securityquestions' )->setQueryString( 'ref', base64_encode( \IPS\Request::i()->url() ) ) );
			}
