...
class _SparkPost extends \IPS\Email
{
	...
	public function _send( $to, $cc=array(), $bcc=array(), $fromEmail = NULL, $fromName = NULL, $additionalHeaders = array() )
	{
		...
		$htmlContent		= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $this->compileContent( 'html', static::_getMemberFromRecipients( $to ) ) ) );
		$plaintextContent	= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $this->compileContent( 'plaintext', static::_getMemberFromRecipients( $to ) ) ) );
		$subject			= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $this->compileSubject( static::_getMemberFromRecipients( $to ) ) ) );
	...
	public function mergeAndSend( $recipients, $fromEmail = NULL, $fromName = NULL, $additionalHeaders = array(), \IPS\Lang $language )
	{
		...
		/* Perform replacements */
		$htmlContent		= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $htmlContent, $varNames ) );
		$plaintextContent	= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $plaintextContent, $varNames ) );
		$subject			= str_replace( array( '__|', '|__' ), array( '{{{', '}}}' ), static::_escapeTemplateTags( $subject, $varNames ) );
