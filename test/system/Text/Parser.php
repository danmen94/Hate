...
class _Parser
{
	...
	protected static function _oembedEmbed( \IPS\Http\Url $url, $iframe=FALSE )
	{
		...
		if ( array_key_exists( $domain, $oembedServices ) )
		{
			...
			if ( is_array( $oembedServices[ $domain ][0] ) )
			{
				foreach ( $oembedServices[ $domain ] as $regex => $endpoints )
				{
				...
				if( $endpoint === NULL )
				{
					$endpoint	= $oembedServices[ $domain ][0][0];
					$endtype	= $oembedServices[ $domain ][0][1];
				}
			}
			else
			{
				$endpoint = $oembedServices[ $domain ][0];
				$endtype  = $oembedServices[ $domain ][1];
			}
