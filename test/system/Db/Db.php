...
class _Db extends \mysqli
{
	...
	public static function i( $identifier=NULL, $connectionSettings=array() )
	{
		...
		if( !isset( static::$multitons[ $identifier ] ) )
		{
			...
			{
				$errorNumber = static::$multitons[ $identifier ]->connect_errno;
				unset( static::$multitons[ $identifier ] );
			...
			if ( \IPS\IN_DEV )
			{
				static::$multitons[ $identifier ]->query( "SET sql_mode='STRICT_ALL_TABLES,ONLY_FULL_GROUP_BY,ANSI_QUOTES'" );
			}
	...
	public function preparedQuery( $query, array $_binds )
	{
		...
		{	
			/* Log */
			$this->log( static::_replaceBinds( $queryForLog, $binds ) );	
		}
